...
export const DeliCard = ({ data, barcode, startDate, endDate }: DeliCardProps) => {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < data.length; i += groupSize) {
            ...
            if (chunk[0] == undefined) { chunk[0] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[1] == undefined) { chunk[1] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[2] == undefined) { chunk[2] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[3] == undefined) { chunk[3] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[4] == undefined) { chunk[4] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[5] == undefined) { chunk[5] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[6] == undefined) { chunk[6] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[7] == undefined) { chunk[7] = { "barcode": 0, "itemName": "", "price": 0 } }
            if (chunk[8] == undefined) { chunk[8] = { "barcode": 0, "itemName": "", "price": 0 } }
                        ...
                        return (
                            <div className="items-center rounded-lg" key={index} style={{ marginBottom: '7px' }}>
                                <div className={`border-4 ${borderColor} h-56 bg-white rounded-lg`}>
                                    <div className="flex h-14"> {/* Set a fixed height for the flex container */}
                                        <div className="w-1/6 flex items-center justify-center"> {/* Center the logo vertically and horizontally */}
                                            {item?.itemName.toString().length !== 0 && (
                                                <img className="mt-2 ml-1 w-85" src={logo} alt="logo" />
                                            )}
                                        </div>
                                        <div className={`w-5/6 text-center font-bold flex items-center justify-center ${item?.itemName.toString().length > 40 ? 'text-[14px]' : 'text-[18px]'}`}>
                                            {item?.itemName.replace(weightRegex, '').trim().toUpperCase()}
                                        </div>
                                    </div>

                                    {(item?.itemName.length !== 0) && <> {item?.itemName.toString().length !== 0 && (
                                        <div className="flex">
                                            <div className="w-1/2 pl-1">
                                                <div className="bg-[#FFFF00] p-4 relative h-24 w-44 border-4 border-red-600 rounded-lg flex flex-col items-center justify-center">
                                                    ...
                                                    <div className="text-lg font-bold  mt-2"> {/* Adjust margin for spacing */}
                                                        100G
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div className="relative float-right pr-1">
                                                    <div className="flex flex-col items-end">
                                                        <Barcode
                                                            ...
                                                            displayValue={false}
                                                        />
                                                        <div className={`${item?.barcode.toString().length <= 10 ? 'text-[24px]' : item?.barcode.toString().length <= 12 ? 'text-[20px]' : 'text-[14px]'} font-bold`}>
                                                            {item?.barcode}
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    {(item?.itemName.length !== 0) && <div className="flex items-end mr-1 ml-1 h-18 mb-1">

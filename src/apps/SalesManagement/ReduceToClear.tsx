...
import {
  companySearchFormSchema,
  reduceToClearSearchFormSchema,
  StoreListFormSchema,
} from "@/lib/utils";
...
const ReduceToClear = ({ title, icon }: any) => {
  ...
  const categoryOptions = [
    { label: "Cheese", value: "Cheese" } /* other categories */,
  ];
  const verificationOptions = [
    { label: "Pending", value: "Pending" } /* other statuses */,
  ];
                ...
                <div>
                  <div className="flex justify-between space-x-4 mt-2 pr-4">
                    ...
                    <div className="flex space-x-4 ml-auto">
                      <Button
                        className="btn-green"
                        onClick={() => handleRTCUpdate("approve")}
                      >
                        Accept RTC
                      </Button>
                      <Button
                        className="btn-danger"
                        onClick={() => handleRTCUpdate("reject")}
                      >
                        Reject RTC
                      </Button>
                    </div>
                        ...
                        <ThemeProvider theme={theme}>
                          <DataGrid
                            ...
                            processRowUpdate={async (newRow: any) => {
                              ...
                              const existingRow: any = rows.find(
                              ...
                              if (
                                existingRow &&
                                newRow.approvedPrice !==
                                  existingRow.approvedPrice &&
                                changedColumns.includes("approvedPrice")
                              ) {
                                ...
                                existingRow &&
                                newRow.price !== existingRow.price &&
                                changedColumns.includes("price")
                              ) {
                                ...
                                if (existingRow.verification !== "Approved") {
                                  ...
                                  setRows((prevRows: any) =>
                                    prevRows.map((row: any) =>
                                      row.id === newRow.id ? existingRow : row
                                    )
                                  );
                                  return existingRow;
                                }
                                ...
                                existingRow &&
                                newRow.qty !== existingRow.qty &&
                                changedColumns.includes("qty") &&
                                existingRow.verification == "Pending"
                              ) {
